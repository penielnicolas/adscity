...
const { checkCaptcha, getServiceFromHost } = require("../middlewares/auth.middleware");
...
exports.whoami = catchAsync(async (req, res) => {
    ...
    if (!token) {
        return res.status(200).json({
            authenticated: false,
            service: getServiceFromHost(req.headers.host)
        });
    }
...
exports.signOut = catchAsync(async (req, res) => {
    ...
    if (token) {
        // Révoquer la session dans tous les services
    ...
    // Nettoyer le cookie sur tous les domaines
    const domains = process.env.NODE_ENV === 'production'
        ? ['.adscity.net', 'adscity.net', 'auth.adscity.net', 'id.adscity.net', 'admin.adscity.net', 'dashboard.adscity.net', 'help.adscity.net', 'pay.adscity.net']
        : ['localhost'];
    ...
    await ActivityService.log(req.user?.id, 'SIGNOUT', 'Déconnexion globale', req);

    res.status(200).json({
        success: true,
        message: 'Déconnexion réussie de tous les services'
    });
});
